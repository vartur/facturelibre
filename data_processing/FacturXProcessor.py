...
class FacturXProcessor:
    ...
    def generate_facturx_xml(cls, template_data: dict[str, Any],
        ...
        invoicer_legal_org = LegalOrganization(id=template_data["invoicer_siren"].strip(),
                                               trading_business_name=template_data[
                                                   "invoicer_trade_name"] if invoicer_has_trade_name else None)

        seller = TradeParty(global_ids=[("0009", template_data["invoicer_siret"].strip())],
                            name=template_data["invoicer_name"],
                            specified_legal_organisation=invoicer_legal_org, trade_address=invoicer_trade_address,
                            specified_tax_registration=template_data[
                                "invoicer_vat_number"].strip() if collect_vat else None,
        ...
        buyer = TradeParty(name=template_data["client_name"],
                           specified_legal_organisation=buyer_legal_org if client_is_pro else None,
                           trade_address=buyer_trade_address,
                           specified_tax_registration=template_data[
                               "client_vat_number"].strip() if client_is_pro and collect_vat else None)
        ...
        for invoiced_item in template_data["invoiced_items"]:
            ...
            applicable_vat_rate_str = invoiced_item["vat_rate"] if collect_vat else 0.0
            trade_tax = cls.VAT_RATES[str(applicable_vat_rate_str)]
            line_item = SupplyChainTradeLineItem(specified_trade_product=TradeProduct(name=invoiced_item["name"]),
            ...
            applicable_trade_taxes.append((line_total_amount, float(invoiced_item["vat_amount"]), trade_tax))
        ...
        if template_data["cash_accepted"]:
            payment_means.append(
                TradeSettlementPaymentMeans(payment_means_code=PaymentMeansCode.CASH, information="En espèces"))

        if template_data["cheques_accepted"]:
            payee = template_data["payee"]
            payment_means.append(TradeSettlementPaymentMeans(payment_means_code=PaymentMeansCode.CHEQUE,
                                                             information=f"Par chèque à l'ordre de {payee}"))

        if template_data["bank_transfers_accepted"]:
            payment_means.append(TradeSettlementPaymentMeans(payment_means_code=PaymentMeansCode.SEPA_CREDIT_TRANSFER,
        ...
        trade_agreement = HeaderTradeAgreement(seller_trade_party=seller, buyer_trade_party=buyer,
                                               contract_referenced_document=contract_doc if template_data[
                                                   "display_contract_number"] else None)

        payment_period_days = template_data["payment_period_days"]
        business_days = "ouvrés" if template_data["business_days"] else ""
        trade_payment_terms = TradePaymentTerms(due_date=datetime.strptime(template_data["payment_date"], "%d/%m/%Y"),
        ...
        for basis_amount, vat_amount, trade_tax in applicable_trade_taxes:
            existing_vat = next(
                (x for x in vat_breakdown if x.rate_applicable_percent == trade_tax.rate_applicable_percent), None)
            if existing_vat:
                existing_vat.basis_amount = existing_vat.basis_amount + basis_amount
                existing_vat.calculated_amount = existing_vat.calculated_amount + vat_amount
            else:
                vat_breakdown.append(TradeTax(type_code=trade_tax.type_code, category_code=trade_tax.category_code,
                                              rate_applicable_percent=trade_tax.rate_applicable_percent,
                                              basis_amount=basis_amount, calculated_amount=vat_amount))

        trade_settlement = HeaderTradeSettlement(invoice_currency_code=template_data["currency_code"],
        ...
        transaction = SupplyChainTradeTransaction(included_supply_chain_trade_line_items=line_items,
